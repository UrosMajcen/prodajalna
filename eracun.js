...
streznik.get('/', function(zahteva, odgovor) {
  //obstaja izrana stranka, prikaži prvotno stran
  ...
  // izbrana ni nobena stranka, preusmeri na prijavo
   else {
...
streznik.post('/izpisiRacunBaza', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    strankaIzRacuna(polja.seznamRacunov, function(kupec) {
...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    try {
      ...
      stmt.finalize();
      izpis = "Stranka je bila uspešno registrirana.";
    } catch (err) {
      napaka2 = true;
      izpis = "Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova.";
    }
    
    if(!napaka2){
    vrniStranke(function(napaka1, stranke) {
      vrniRacune(function(napaka2, racuni) {
        odgovor.render('prijava', {sporocilo: izpis, seznamStrank: stranke, seznamRacunov: racuni});  
      });
    });
    }    
  });
